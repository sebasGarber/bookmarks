                        ...
                        <div className={classes.bookmarkContainer}>
                            <div className={classes.btnCont}>
                                ...
                                {!showConfirm ? <button onClick={ () => removeCategory() }>
                                    ...
                                    <span>יש {showConfirm} bookmarks, בטוח למחוק הכל?</span><br/>
                                    <button style={{color: 'red'}} onClick={ () => removeMe() } >אישור</button>{' | '}<button onClick={ ()=> setShowConfirm(false) }>ביטול</button>
                                </div> }
...
function EditBookmark(props) {
    ...
    return(
        ...
        <Button className={`customBtn_violet bounce3`} disabled={loading} variant={'primary'} size="lg" onClick={ onSubmit }>
            {loading ? <Spinner size="" animation="border" variant="light" /> : <span>עריכה &raquo;</span> }
        </Button>
